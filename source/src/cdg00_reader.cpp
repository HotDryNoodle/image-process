...
void CDG00Reader::RecalculateParameters() {
    ...
    GST_DEBUG(
        "CDG00Reader parameters recalculated: "
        "channel=%d, stride_lines=%u, image_width=%u, image_height=%u, "
        "block_size=%zu, stride_offset=%lld",
        channel_, stride_lines_, image_width_, image_height_, block_size_,
        (long long)stride_offset_);
}
...
PropertyDefinitions CDG00Reader::GetPropertyDefinitions() const {
    ...
    stride_def.default_value =
        PropertyValue(static_cast<int>(kDefaultStrideLines));
    stride_def.description = "Number of lines to stride per read";
    stride_def.min_value   = PropertyValue(1);
    stride_def.max_value =
        PropertyValue(16384);  // 允许 stride==image-height 即不重叠开窗
    ...
    height_def.default_value =
        PropertyValue(static_cast<int>(kDefaultImageHeight));
...
bool CDG00Reader::SetProperty(const std::string&   name,
    ...
    if (name == "channel") {
        ...
        if (stride_val < 1 || stride_val > 16384) {
        ...
        if (fps_val < 0.1f || fps_val > 1000.0f) {
            GST_WARNING(
                "Invalid fps value: %f (must be between 0.1 and 1000.0)",
                fps_val);
            return false;
        }
        fps_ = fps_val;
...
PropertyValue CDG00Reader::GetProperty(const std::string& name) const {
    if (name == "channel") { return PropertyValue(static_cast<int>(channel_)); }
    else if (name == "stride-lines") {
    ...
    else if (name == "fps") { return PropertyValue(fps_); }
...
GstCaps* CDG00Reader::GetCaps() {
    ...
    gst_video_info_set_format(&info, GST_VIDEO_FORMAT_GRAY8, image_width_,
                              image_height_);
...
GstBuffer* CDG00Reader::ProcessFrame(const guint8* data,
    ...
    // Allow processing of partial frames (e.g., last frame in file may be
    // incomplete) Calculate line size to determine minimum required data

    if (size < kLineDataSize) {
        GST_WARNING(
            "Data size %zu is less than one complete line (%zu bytes), cannot "
            "process",
            size, kLineDataSize);
        return nullptr;
    }
    ...
    if (size < block_size_) {
        GST_INFO(
            "Partial frame detected: got %zu bytes, expected %zu (processing "
            "available data)",
            size, block_size_);
    }
...
// Note: SetBinFileSrcProperty removed - BinFileSrc now queries via
// GetBlockSize/GetStrideOffset
...
gsize CDG00Reader::GetBlockSize() const { return block_size_; }

gint64 CDG00Reader::GetStrideOffset() const { return stride_offset_; }

bool CDG00Reader::CheckLineHeader(const guint8* data, guint8& row_num) {
    if (!data) { return false; }
    // check header
    if (memcmp(data, kMagicNumber, 5) != 0) { return false; }
    row_num = (data[9] & 0x0F);
...
CDG00Parameter CDG00Reader::PaserMetaData(const guint8* data) {
    ...
    uint8_t arry[4]    = {ptr_line14[30], ptr_line14[31], ptr_line15[0],
                          ptr_line15[1]};
...
GstBuffer* CDG00Reader::ProcessVideoDataChannel(const guint8* data,
    ...
    if (num_lines == 0) {
        GST_WARNING(
            "Data size %zu too small for even one line (expected %zu per line)",
            size, kLineDataSize);
        return nullptr;
    }

    // Prepare output buffer for entire frame (all lines converted to 8-bit)
    const gsize frame_output_size =
        image_width_ * num_lines;  // 8-bit per pixel
    ...
    for (guint line = 0; line < num_lines; ++line) {
        ...
        const guint8* line_video_data =
            current_line_ptr + kLineHeaderSize + kLineParamSize;

        // Convert 10-bit line data to 8-bit
        const gsize line_converted =
            Convert10bitTo8bit(line_video_data, line_video_size,
        ...
        if (line_converted == 0) {
            ...
            GST_DEBUG("Converted line %u: %zu bytes -> %zu bytes", line,
                      line_video_size, line_converted);
        }
    ...
    GstBuffer* buffer =
        gst_buffer_new_allocate(nullptr, expected_frame_size, nullptr);
    if (!buffer) {
        GST_ERROR("Failed to allocate GstBuffer of size %zu",
                  expected_frame_size);
        return nullptr;
    }
    ...
    if (gst_buffer_map(buffer, &map, GST_MAP_WRITE)) {
        ...
        if (total_converted < expected_frame_size) {
            ...
            GST_INFO(
                "Padded partial frame: %zu bytes converted, %zu bytes padded "
                "(total %zu)",
    ...
    GST_DEBUG(
        "Successfully processed frame: %u lines converted, %zu total bytes "
        "output",
    ...
    // time stamp: meta_cdg00.params[0].expose_time_ns(纳秒) X
    // meta_cdg00.params[0].row_num GstClockTime pts  =
    // meta_cdg00.params[0].row_num * meta_cdg00.params[0].expose_time_ns;
    // GstClockTime dur  = image_height_ * meta_cdg00.params[0].expose_time_ns;
    // // 如果一帧只有一行，n_rows=1 GST_BUFFER_DURATION(buffer) = dur; add meta
    if (!MetaFactory::AddMetaToBuffer<MetaCDG00Impl>(buffer, meta_cdg00)) {
        GST_WARNING("Failed to add cdg00 meta to buffer");
    }
    else { GST_DEBUG("Added cdg00 meta buffer"); }
    return buffer;
}
...
gsize CDG00Reader::Convert10bitTo8bit(const guint8* input_data,
    ...
    const gsize max_output_pixels =
        std::min(output_size, num_packs * pixels_per_pack);

    gsize output_index = 0;

    for (gsize pack = 0; pack < num_packs && output_index < max_output_pixels;
         ++pack) {
        ...
        if (output_index < max_output_pixels)
            output_buffer[output_index++] = p0 >> 2;
        if (output_index < max_output_pixels)
            output_buffer[output_index++] = p1 >> 2;
        if (output_index < max_output_pixels)
            output_buffer[output_index++] = p2 >> 2;
        if (output_index < max_output_pixels)
            output_buffer[output_index++] = p3 >> 2;
    }
